...
public class BabyName
{
    ...
    ///
    /// <summary>
    /// Compares THIS object's name to the given obj's name, ignoring case.
    /// Overriding Equals and GetHashCode enables searching in collections.
    /// </summary>
    /// <param name="obj">the object to compare to</param>
    /// <returns>true if objects are same type and have same name, false if not</returns>
    ///
    public override bool Equals(object obj)
    {
        if (obj == null || obj.GetType().Equals(this.GetType()) == false)
            return false;  // cannot compare apples and oranges!

        return this.Name.Equals(((BabyName)obj).Name, StringComparison.OrdinalIgnoreCase);
    }

    /// <summary>
    /// Returns an appropriate hash code for this name.
    /// </summary>
    public override int GetHashCode()
    {
        //
        // rule: if two objects are Equals, GHC must return same value, otherwise the value
        //   returned doesn't matter.  The best way to do this is to compute GHC based on 
        //   the same data being compared by Equals.
        //
